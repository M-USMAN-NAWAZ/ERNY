...
namespace SilverTau.NSR
{
    ...
    public class NSREditor : NSRPackageEditor
    {
		...
		public static void PrepareAndCheckNSRScreenRecorderiOS()
		{
			...
			EditorUtility.DisplayDialog("NSR - Screen Recorder", "Screen Recorder (iOS) is ready to use.", "OK");
		}
		...
		public static void PrepareAndCheckNSRScreenRecorder()
		{
			...
			CheckAudioSettings();
			NSRDefineSymbolsInstaller.EnsureBasePerGroupDefines();
			
			Debug.Log("<color=cyan>NSR - Screen Recorder - Universal Video Recorder is ready to use.</color>");
			EditorUtility.DisplayDialog("NSR - Screen Recorder", "Universal Video Recorder is ready to use.", "OK");
		}
		...
		public static void PrepareAndCheckNSRMicrophoneAudioRecorder()
		{
			...
			EditorUtility.DisplayDialog("NSR - Screen Recorder", "Microphone Audio Recorder (Universal) is ready to use.", "OK");
		}
        
		[MenuItem("Window/Silver Tau/NSR - Screen Recorder/Validate plugin", false, 2)]
		public static void ValidateNSRScreenRecorder()
		{
			...
			var files = new List<string> {"SilverTau_NSR.framework", "SilverTau_NSR.dll", "SilverTau_NSR.bundle", "SilverTau_NSR.aar", "NSR.framework", "NSR.aar", "SilverTau_NSR_Core.dll"};
			...
			foreach (var asset in assets)
			{
				...
				CheckAssetInProject(path, files, "SilverTau_NSR.framework", ref refValues);
				CheckAssetInProject(path, files, "SilverTau_NSR.dll", ref refValues);
				CheckAssetInProject(path, files, "NSR.aar", ref refValues);
				CheckAssetInProject(path, files, "SilverTau_NSR.bundle", ref refValues);
				CheckAssetInProject(path, files, "SilverTau_NSR.aar", ref refValues);
				CheckAssetInProject(path, files, "SilverTau_NSR_Core.dll", ref refValues);
			}
			...
			if (!hasWarning)
			{
				Debug.Log("<color=cyan>NSR - Screen Recorder is ready to use.</color>");
				EditorUtility.DisplayDialog("NSR - Screen Recorder", "NSR - Screen Recorder is ready to use.", "OK");
			}
		}
		
		private static void CheckAudioSettings(int sampleRate = 48000)
		{
	    ...
	    {
		    var pathFile = Path.GetFileName(path);
		    
		    if (pathFile != assetName) return;
		    
		    if (pathFile == (assetName + ".meta")) return;

		    var firstAsset = files.FirstOrDefault(s => s == assetName);
    ...
    [InitializeOnLoad]
    public static class NSREditorConfiguration
    {
	    static NSREditorConfiguration()
	    {
	    ...
	    private static void CheckAudioConfiguration(int sampleRate = 48000)
	    {
		    var audioConfig = AudioSettings.GetConfiguration();
