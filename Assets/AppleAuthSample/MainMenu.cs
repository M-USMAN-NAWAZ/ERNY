...
namespace AppleAuthSample
{
    public class MainMenu : MonoBehaviour
    {
        ...
        string yourIdToken = "\"eyJraWQiOiJTZjJsRnF3a3BYIiwiYWxnIjoiUlMyNTYifQ..T3re4LJjD3xR-bR4AHwG3Sd5RHY5Ej8PfDlAZOA6plaoqIGl_ol-sYfjWN1vO3KiPNlNq8bCg6wGu9-uv-rlvKrMd2yNzdq8rfD4NVHjFY6n60o_CulTGLKWmQ42pj-49KdI9W3_JOLzB8RmIYK8JaCsDSbS0tSk7n5f71T9zqWQszB8X451YyZOaHtRzt0ZLo0wkuhj3MOMcA3FyXqAF1fBrT08aJ08dRx5mzlqoLQpFZnNE6pSKpb9thWU8eFA09OvXXv4EYtec2sdO44gYgW43BTDMJcH6faSpEcjtGqJ7AisUB7-ufa8SZcIoi51JfUH1y0rcmTrEYQuytAexA\"";

        public string userEmail;
        public string AppleIdTokenData;
        ...
        {
            IfChechInitialize();
            //this.SetupLoginMenuForAppleSignIn();//// view 
        ...
        public void SetupGameMenu(string appleUserId, ICredential credential)
        {
        ...
        private void SignInWithApple()
        {
            ...
            this._appleAuthManager.LoginWithAppleId(
                ...
                credential =>
                {
                    ...
                    this.SetupGameMenu(credential.User, credential);
                    this.GameMenu.SetVisible(visible: true);
                    Debug.Log("IdToken is: " + this.GameMenu.AppleIdTokenData);
                    userEmail = GameMenu.userEmail;


                    AppleIdTokenData = GameMenu.AppleIdTokenData;
                    ApiRequestGenerator.LoginWithApple(GameMenu.AppleIdTokenData);

                    ApiRequestGenerator.loading.SetActive(true);

                    PlayerPrefs.SetString(AppleUserIdKey, GameMenu.AppleIdTokenData);
                },
                error =>
                {
                    var authorizationErrorCode = error.GetAuthorizationErrorCode();
        ...
        public void GetAppleIdTokenOnly()
        {
            string _idToken = PlayerPrefs.GetString(AppleUserIdKey);

            AppleIdTokenData = _idToken;
            ApiRequestGenerator.LoginWithApple(_idToken);

            Debug.Log("The Id token is: " + _idToken);
        }



        public void LogoutFromApple()
        {
            // 1. Clear stored Apple credentials
            if (PlayerPrefs.HasKey(AppleUserIdKey))
            {
                PlayerPrefs.DeleteKey(AppleUserIdKey);
            }

            // 2. Reset UI back to login
            this.SetupLoginMenuForSignInWithApple();
            this.LoginMenu.SetVisible(true);
            this.GameMenu.SetVisible(false);

            Debug.Log("User logged out from Apple Sign In (local data cleared).");
        }
